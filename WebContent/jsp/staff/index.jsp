...
<div class="page-content">
      <div class="page-header fixed-div">
        <p>
          <lable>员工编号：</lable><input type="text" id="id"/>
          <lable>员工名字：</lable><input type="text" id="name"/>
          <lable>员工职务：</lable><input type="text" id="post"/>
        </p>
        <p>
              ...
              <table id="table" class="table table-striped table-bordered table-hover dataTable" aria-describedby="sample-table-2_info"> 
               <thead> 
                <tr role="row"> 
                 ...
                 <th  role="columnheader"  rowspan="1" colspan="1" style="width: 153px;" >员工编号</th> 
                 ...
                 <th role="columnheader"  rowspan="1" colspan="1" style="width: 130px;" >年龄</th>
                 <th role="columnheader"  rowspan="1" colspan="1" style="width: 130px;" >职务</th>
	...
	jQuery(function($) {
	    ...
	    $("#search").click(function () {
	           ...
	           var name=$("#name").val();
	           var post=$("#post").val();
	           gotoPage(1,"id="+id+"&name="+name+"&post="+post);
	    });
	    
		/* 获取数据 */
		gotoPage(1,"id=&name=&post=");
			...
			success : function(msg) {
				if(msg.page.totalElement == 0){
					...
					$.each(msg.page.content, function(i, item) {
			              $('#tb').append( "<tr>"
			            		  +"<td><label> <input type='checkbox' class='ace' name='checkbox' value='"+item.staffDetailId+"' /><span class='lbl'></span> </label></td>"
			            		  +"<td >"+(++i)+"</td> "
			            		  +"<td >"+item.tbStaff.staffId+"</td> "
			            		  +"<td >"+item.tbStaff.staffName+"</td> "
			            		  +"<td >"+showinfo(item.tbStaff.staffSex)+"</td> "
			            		  +"<td >"+item.tbStaff.staffAge+"</td> "
			            		  +"<td >"+item.tbStaff.staffPost+"</td> "
			            		  +"<td >"+item.tbStaff.staffPhone+"</td> "
			            		  +"<td >"+"<div class='visible-md visible-lg hidden-sm hidden-xs action-buttons' id='buttontools'>"
			            		  				+"<a class='green' href='staffDetail/showOne.html?id="+item.id+"' > <i class='icon-pencil bigger-130'></i> </a>"
			            		  				+"<a class='red' href='staffDetail/delete?id="+
			            		  											item.tbStaff.staffId+
			            		  											"&name="+item.tbStaff.staffName+
			            		  											"&sex="+item.tbStaff.staffSex+
			            		  											"&age="+item.tbStaff.staffAge+
			            		  											"&post="+item.tbStaff.staffPost+
			            		  											"&phone="+item.tbStaff.staffPhone+
			            		  											"&card="+item.staffCard+
			            		  											"&address="+item.staffAddress+
			            		  											"&email="+item.staffEmail+
			            		  											"&time="+item.staffTime+
			            		  											"&staffDetailId="+item.id+"' > <i class='icon-trash bigger-130'></i> </a>"
			            		  				+"</td> "+"</tr>");
			            });
